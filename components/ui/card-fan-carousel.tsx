...
export default function SocialCards({ cards }: SocialCardsProps) {
  ...
  useEffect(() => {
    ...
    cardElements.forEach((card, cardIndex) => {
      ...
      if (slot !== undefined) {
        ...
        if (isFirstMount) {
          gsap.set(card, { x: 0, y: `${12 * hMult}rem`, xPercent: -50, yPercent: -50, rotation: 0, scale: 0.5, opacity: 0 });
          gsap.to(card, { ...target, duration: 1.2, ease: "elastic.out(1.05,.78)", delay: 0.2 + slot * 0.06, onComplete: onCardDone });
        } else if (!wasVisible) {
          const enterX = direction === "right" ? 40 : -40;
          gsap.set(card, { x: `${enterX}rem`, y: `${y * hMult}rem`, xPercent: -50, yPercent: -50, rotation: direction === "right" ? 30 : -30, scale: 0.5, opacity: 0 });
        ...
        gsap.to(card, { x: `${exitX}rem`, xPercent: -50, yPercent: -50, opacity: 0, scale: 0.5, rotation: direction === "right" ? -30 : 30, duration: 0.4, ease: "power2.in", zIndex: 0 });
      } else if (isFirstMount) {
        gsap.set(card, { opacity: 0, scale: 0.3, x: 0, y: 0, xPercent: -50, yPercent: -50, zIndex: 0 });
      }
    });
    ...
    const updateHoverLayout = (hoveredSlot: number | null) => {
      ...
      visibleEntries.forEach(({ el, slot }) => {
        ...
        gsap.to(el, {
          x: `${targetX}rem`, y: `${targetY}rem`, xPercent: -50, yPercent: -50, rotation: targetRot, scale: targetScale,
  ...
  }, [cycle, needsPagination, centerIndex, totalCards]);
  ...
  return (
    <section className="flex flex-col items-center w-full py-4 lg:py-8 px-4 md:px-8 relative z-20">
      <div className="flex items-center justify-center w-full max-w-[90rem]">
        <div ref={containerRef} className="fan-layout flex relative justify-center items-center w-full max-w-[70rem] min-h-[24rem] sm:min-h-[26rem] md:min-h-[30rem]">
      ...
      {needsPagination && (
        <div className="flex items-center justify-center gap-4 mt-8 md:mt-12 z-30 over">
          <button className={`${ARROW_CLASSES} w-10 h-10 md:w-12 md:h-12`} onClick={() => cycle("left")} aria-label="Previous">
            {chevron("left")}
          </button>
          <div className="flex items-center gap-2">
            {cards.map((_, i) => (
              <span key={i} className={`w-2 h-2 rounded-full transition-all duration-300 ${i === centerIndex ? "bg-primary scale-[1.3]" : "bg-muted-foreground/30"}`} />
            ))}
          </div>
