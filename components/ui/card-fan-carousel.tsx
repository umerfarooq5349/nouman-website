  ...
  return (
    <section className="flex flex-col items-center w-full py-4 lg:py-8 px-4 md:px-8 relative z-20">
      <div className="flex items-center justify-center w-full max-w-[90rem]">
        <div ref={containerRef} className="fan-layout flex relative justify-center items-center w-full max-w-[80rem] min-h-[30rem]">
          {cards.map((card, index) => {
            const image = (
              <div className="relative w-[12rem] h-[21rem] md:w-[15rem] md:h-[26rem] overflow-hidden rounded-2xl border border-border/40 shadow-xl bg-card">
            ...
            return card.linkUrl ? (
              <a key={index} href={card.linkUrl} onClick={handleClick} className="fan-card absolute cursor-pointer">{image}</a>
            ) : (
